...
public class CamundaQueryService implements ProcessEngineQueryAPI {
  ...
  @Override
  public TaskVO queryRelateTaskById(String taskId, Long userId, List<Long> roleIdList) {
    List<String> identityList = generateIdentityList(userId, roleIdList);
  ...
  @Override
  public List<TaskVO> queryRelateTaskList(Long userId, List<Long> roleIdList) {
    List<String> identityList = generateIdentityList(userId, roleIdList);
   ...
   * 获取 identityList
   */
  private List<String> generateIdentityList(Long userId, List<Long> roleIdList) {
    List<String> identityList = Lists.newArrayList();
    if (userId != null) {
      identityList.add(userId.toString());
    }
    if (CollectionUtil.isNotEmpty(roleIdList)) {
      identityList.addAll(roleIdList.stream()
          .map(String::valueOf)
